...
`z3rno-helm` is a Helm chart that deploys the full Z3rno stack on Kubernetes: the API server, PostgreSQL with extensions, Redis, and Celery workers.
...
| `redis` | 1 | Celery broker + rate limit store (Deployment) |
