...
- **Forge** (`ingest`, `distill`, `refine`) — the structure-building layer added in Phases A–D. Today these are HTTP-only; SDK wrappers ship as part of Phase E.
...
```bash
# Text + URL ingest
curl -X POST https://api.z3rno.dev/v1/ingest \
...
```bash
curl -X POST https://api.z3rno.dev/v1/distill \
  -H "Authorization: Bearer sk-..." \
  -d '{
    "agent_id": "agent-1",
    "memory_ids": ["01HXY1...", "01HXY2..."]
  }'
```

Returns `202` + `job_id`; poll `GET /v1/distill/{job_id}` for status.
...
```bash
curl -X POST https://api.z3rno.dev/v1/refine \
  -H "Authorization: Bearer sk-..." \
  -d '{"dataset_id": "01HXYZ..."}'
```
...
| `forget` | ✅ | ✅ | ✅ |
| `audit` | ✅ | ✅ | ✅ |
| `ingest` | — *(Phase E)* | — *(Phase E)* | ✅ |
| `distill` | — *(Phase E)* | — *(Phase E)* | ✅ |
| `refine` | — *(Phase E)* | — *(Phase E)* | ✅ |

The Forge verbs (`ingest` / `distill` / `refine`) are operator-flagged on the server (`INGEST_ENABLED`, `DISTILL_ENABLED`, `REFINE_ENABLED`) and dormant by default. Until they're enabled, those routes are not even registered — OpenAPI byte-identical, calls return 404.
